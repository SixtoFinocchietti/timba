...
| ✅ 8.2 Asistencia por jugador al crear partida online ("hándicap", §2.3) | Media | Migración `025_pool_asistencia_por_jugador.sql` (`partidas_pool.asistencia_host`/`asistencia_invitado`, default `'normal'`, aplicada). El host fija ambos niveles al armar la invitación (`pool-online.tsx`, dos selectores independientes de `NIVELES_ASISTENCIA`); viajan en el `contenido` del mensaje, se escriben al crear la fila (`sala-pool.tsx`) y quedan visibles para los dos en la card de "Configuración de la partida" antes de arrancar. `partida-pool.tsx` calcula `nivelAsistenciaEfectivo` (mi asiento → mi columna) y lo usa en vez del default personal SOLO en online — práctica/bot siguen usando el default de siempre |
