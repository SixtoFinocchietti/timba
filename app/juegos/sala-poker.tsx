...
import { estadoInicial } from '@/lib/poker'
...
export default function SalaPoker() {
  const c = useColores()
  ...
  const modoLabel = MODO_LABELS[params.modo] ?? 'En vivo'
  const limiteLabel = LIMITE_LABELS[params.limite] ?? 'Sin límite'
  const fichasNum = parseInt(params.fichas ?? '5000', 10)
  const fichasLabel = formatFichas(fichasNum)

  const subtitulo = `${limiteLabel} · ${fichasLabel} fichas · ${modoLabel}`
  ...
  useEffect(() => {
    ...
    const canal = supabase.channel(`sala-poker-${roomKey}`)
  ...
  // Invitado: escucha el INSERT en partidas_poker para navegar al juego automáticamente
  useEffect(() => {
    if (!esInvitado || !usuario?.id) return
    const canal = supabase
      .channel('sala-poker-inicio')
      .on('postgres_changes', {
        event: 'INSERT', schema: 'public', table: 'partidas_poker',
        filter: `invitado_id=eq.${usuario.id}`,
      }, payload => {
        router.replace({ pathname: '/juegos/partida-poker', params: { partidaId: payload.new.id } } as any)
      })
      .subscribe()
    return () => { supabase.removeChannel(canal) }
  ...
  async function reenviarInvitacion() {
    ...
    const contenido = JSON.stringify({
      modo: params.modo ?? 'vivo',
      jugadores: parseInt(params.jugadores ?? '2', 10),
      limite: params.limite ?? 'sinLimite',
      fichas: fichasNum,
      hostId: usuario.id,
  ...
  async function empezarPartida() {
    ...
    router.replace({ pathname: '/juegos/partida-poker', params: { partidaId: data.id } } as any)
  }
  ...
  return (
    <View style={[es.contenedor, { backgroundColor: c.fondo }]}>
      <View style={es.header}>
        ...
        <Text style={[es.tituloHeader, { color: c.texto }]}>Sala de Póker</Text>
        <View style={{ width: 18 }} />
      </View>
      ...
      <View style={es.mesaWrap}>
        <View style={[es.mesaOval, { borderColor: c.borde }]}>
          ...
          <View style={es.centro}>
            ...
            <Text style={[es.boteLabel, { color: c.textoSuave }]}>BOTE</Text>
            <Text style={[es.boteValor, { color: c.primario }]}>0</Text>
